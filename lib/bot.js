...
function Bot(controller, slackToken, authCache) {
    ...
    return function(reply, private) {
      ...
      if(private) {
        bot.api.im.open({user: message.user}, function (err, response) {
          ...
          replyObj.channel = response.channel.id;
          bot.say(replyObj);
        });
      } else {
  ...
  function showDigest(user, channel) {
    ...
    botData.getUserChannelData(user, channel).then(function(userData) {
      if(!userData) { userDataErrorHandler(say, userData); }
  ...
  controller.hears(['gcpbot d(eploy)? detail (.*)'], ['message_received','ambient'], function (bot, message) {
    ...
    botData.getUserChannelData(message.user, message.channel).then(function(userData) {
      if(!userData) { userDataErrorHandler(gcpClient.replier, userData); }
