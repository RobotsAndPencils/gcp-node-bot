...
GCPClient.prototype.authorize = function() {
  ...
  var auth = self.authCache.lookupAuth(self.user);
  if(auth.client && auth.client.credentials && auth.client.credentials.access_token) {
    self.auth = auth;
    return auth;
  }
  if(!auth.client) {
    self.auth = self.authCache.createAuth(self.user);
    var url = self.authCache.generateAuthUrl(self.user);
    replier("🔑 Log into your Google Cloud Platform account here to give me authorization: " + url, true);
  }
  replier("🔑 Before I can help you, you need to give me authorizion to use your Google Cloud Platform account. I sent you a direct message with the link.");
  return null;
};
...
GCPClient.prototype.showDeployList = function(userData) {
  ...
  if(this.authorize()) {
    ...
    return listDeployments(userData, client).catch(function(err) {
...
GCPClient.prototype.showDeploySummary = function(userData, email) {
  ...
  if(this.authorize()) {
    ...
    return listDeployments(userData, client, filterStr).catch(function(err) {
      client.replier("🚫 There was an error listing deployments.");
    });
  } else {
    return Promise.resolve();
...
GCPClient.prototype.newDeploy = function(userData, repo, depFile) {
  ...
  return fetchConfiguration(baseURL, yamlName).then(function(result) {
    ...
    if(client.authorize()) {
      var insert = insertDeployment(client, userData, depFile, configString, result.imports);
...
GCPClient.prototype.listMetrics = function(userData, metricFilters) {
  ...
  if(this.authorize()) {
    return monitoring.projects.metricDescriptors.list({
